...
describe("Reserve one ticket first option selected full pay, then deposit pay",()=>{
    ...
    it("one ticket with  first option selected full pay, then deposit pay, tandem package & selected video are not taxble and weekend",async()=>{
        let numOfTickets = 2;
        const videoOptions = ["NT","NT"];
        const tandemPackages = ["NT","NT"];
        let payOption = "full";
        ...
        await Reservation_Testing_Utils.decrease_Num_ticekts();
        await browser.pause(2500);
