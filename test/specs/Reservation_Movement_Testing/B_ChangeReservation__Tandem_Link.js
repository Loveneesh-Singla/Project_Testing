...
describe("create tandem reservation, change reservation from email link and verify the timeslots",()=>{
    it("create tandem reservation, change reservation from email link and verify the timeslots" , async()=>{
        ...
        const jumperNames = await Reservation_Testing_Utils.fill_passenger_details(numOfTickets,tandemPackages,videoOptions,false,false,true);
        ...
        const dateSel = await ChangeReservation_Page.get_date_sel(19);
        await dateSel.click();
        await browser.pause(1000);
